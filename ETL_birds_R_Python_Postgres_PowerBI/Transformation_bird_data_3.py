...
bird_data = pd.read_csv("G:/Research data/Other groups/Ebird_data/ETL_birds_R_Python_Postgres_PowerBI/bird_data_extraction.csv")
...
len(bird_data_transform[bird_data_transform.scientific_name.str.contains(pat='sp.$')])
...
bird_data_transform['locality']=bird_data_transform.locality.str.replace('\d+','')
...
bird_data_transform['locality']=bird_data_transform.locality.str.replace('^<u.+','')
...
bird_data_transform['locality']= bird_data_transform['locality'].str.replace('[/._-]+','')

# Select the initial section of the locality

bird_data_transform['locality']=bird_data_transform.locality.str.split(',',n=1).str.get(0)

#bird_data_transform['locality'].str.contains('.+, pune, maharashtra, in$').head(50)

bird_data_transform['locality'] = bird_data_transform['locality'].replace('.+, pune, maharashtra, in','')

# Remove addtional spaces in between the locality names

bird_data_transform.locality.replace(r'\s+',' ',regex=True).head(50)

bird_data_transform['locality'] = bird_data_transform.locality.replace(r'\s+',' ',regex=True)
...
bird_data_transform['locality'] = bird_data_transform.locality.replace(r'',np.NaN,regex=True)

bird_data_transform['locality']=bird_data_transform['locality'].fillna('Location name not known')

# Trimming the blank spaces (if/any) in the text columns

bird_data_transform[column_names] = bird_data_transform[column_names].apply(lambda x:x.str.strip())

# Converting the text columns in appropriate case

bird_data_transform[column_names] = bird_data_transform[column_names].apply(lambda x:x.str.capitalize())
